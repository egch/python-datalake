#!/bin/bash
# Wire Event Grid subscription: storage container → Event Hub for ACJ
set -euo pipefail
...
az eventgrid event-subscription create \
  --name "sub-${ACJ_STORAGE_CONTAINER}" \
  ...
  --subject-begins-with "/blobServices/default/containers/${ACJ_STORAGE_CONTAINER}" \
  --output table
